...
public:
	...
	// Skips whitespace
	// If can't, s an error: unexpected end of file
	...
	// Unless the given string is read, s an error
...
void readIntList(vector<string> &vec, istream &infile)
{
	char p;
	while(true)
	{
...
void readIntList(DummyTreeNode *node, istream &infile, bool invert)
{
	char p;
	while(true)
	{
...
DummyTreeNode *parseBlock(istream &infile)
{
	char c, p;
...
bool parse_qpro1(istream &infile, Circuit &circuit)
{
	char c;
	string s;
...
bool addBlock(string name, istream &infile, Circuit &circuit)
{
	char c, p;
...
bool parse(char * filename, Circuit &circuit)
{
	...
	*************************************************************************/
	...
	if (Common::qpro >= 2 && Common::qpro <= 5)
	{
		QdToQpro * qq = new QdToQpro();
		bool howdidIdo = qq->init(filename);
		if (howdidIdo == false)
		{
			error("QdToQpro::init failed");
			return false;
		}
		if (Common::qpro == 3  || Common::qpro == 5)
		{
			cout << qq->sqpro.str();
			exit(0);
		}
		bool parse_qpro_ok = parse_qpro1(qq->sqpro, circuit);
		delete qq;
		return parse_qpro_ok;
	}
	else if (Common::qpro)
	{
		ifstream infile;
