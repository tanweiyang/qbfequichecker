...
{
		cout << "Usage: solver [-v] [-silent] [-qpro fmt] [-nprep] [-iscas outfile] [-t limit] [-larify]\n";
		cout <<	"         [-phase [exist/univ]] [-proof <problem_file> <proof_file>]\n";
		cout << "         filename [timeout]\n";
		cout << "   Options:\n";
		cout << "      -v        verbose\n";
		cout << "      -silent   suppresses statistics output\n";
		cout << "      -qpro     fmt defines input format: 0=iscas; 1=qpro;\n";
		cout << "                2=qdimacs; 3=qdimacs, print qpro instead of solving\n";
		cout << "                4=qdimacs_naive; 5=qdimacs_naive, prt qpro instead of solving\n";
		...
		cout << "      -nprep    Skip the preprocessing step\n";
	...
	*************************************************************************/
	...
	for( int i = 1; i < argc; i++ )
	{
		if( argv[i][0] == '-' )
		{
			if( argv[i][1] == 'v' )
				verbose = true;
			else if( strcmp( argv[i], "-qpro" ) == 0 )
			{
			        int qproCL = atoi( argv[++i] );
			        if (0 <= qproCL && qproCL <= 5)
			                Common::qpro = qproCL;
			        else
			        {
			                printUsage();
			...
			else if( strcmp( argv[i], "-nprep" ) == 0 )
			{
				Common::preprocess = false;
			}
		...
		else
		{
			...
			else
			{
				...
				solver_time_out = atoi( argv[i]);
				if(solver_time_out <= 0)
				{
					cerr << "Invalid timeout: " << argv[i] << endl;
					return 1;
				}
			}
		}
